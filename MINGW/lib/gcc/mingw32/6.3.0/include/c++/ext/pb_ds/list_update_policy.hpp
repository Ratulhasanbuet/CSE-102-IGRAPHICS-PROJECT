...
#include <cstdlib>
#include <ext/pb_ds/detail/list_update_policy/lu_i_metadata.hpp>
...
namespace __gnu_pbds
{
 ...
 template<typename _Alloc = std::allocator<char> >
   ...
   *  list based on the i algorithm.
   */
  template<std::size_t Max_Count = 5, typename _Alloc = std::allocator<char> >
    class lu_i_policy
    : private detail::lu_i_policy_base<typename _Alloc::size_type>
    {
    public:
      typedef _Alloc 					allocator_type;
      ...
      enum
	{
      ...
      /// Metadata on which this functor operates.
      typedef detail::lu_i_metadata<size_type> 	metadata_type;

    private:
      typedef detail::lu_i_policy_base<size_type> 	base_type;
      typedef typename _Alloc::template rebind<metadata_type> __rebind_m;
